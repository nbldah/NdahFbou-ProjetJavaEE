    ...
    <table>
        ...
        <tr>
            <g:sortableColumn property="nom"
                              title="${message(code: 'musee.nom.label', default: 'Nom')}"/>

            <g:sortableColumn property="horaireOuverture"
                              title="${message(code: 'musee.horaireOuverture.label', default: 'Horaire Ouverture')}"/>
            ...
            <th><g:message code="musee.gestionnaire.label"
                           default="Gestionnaire"/></th>
        ...
        <g:each in="${museeInstanceList}" status="i" var="museeInstance">
            ...
            <tr>
                <g:if test="${!(((List) session.getAttribute("mesFavoris"))?.contains(museeInstance.nom))}">
                    <td><g:link action="ajouterFavoris"
                                params="[nom: museeInstance.nom]"
                                style="text-align: right"><input type="button"
                                                                 value="Ajouter à ma liste"
                                                                 class="button"/></g:link>
                    </td>
                </g:if>
            </tr>
...
<g:if test="${((List) session.getAttribute("mesFavoris"))?.size() > 0}">
    <div>
        ...
        <table>
            ...
            <g:each in="${session.getAttribute("mesFavoris")}" status="i"
                    var="museeInstance">

                <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                    <td>${museeInstance}</td>
                    <td><g:link action="supprimerFavoris"
                                params="[nom: museeInstance, from: 'musee/index']"
                                style="text-align: right">
                        <input type="button" value="Supprimer"></g:link></td>
                    <td><g:link action="redirectToDemandeVisite"
                                controller="musee"
                                style="text-align: right"><input type="button"
                                                                 value="Faire une demande"></g:link>
                    </td>
                </tr>
